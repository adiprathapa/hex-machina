...
| 8 | Agent activity is visible in the interface. | Verified | Shared handlers feed a typed result-presentation channel, activity list, and graph highlighting. Every proposed operation is rendered in a review ledger; removed edges, added edges, and activated runes are simultaneously distinguished on the canvas. The production-browser test invokes all seven registered definitions directly and checks the causal failure, sacred pin, complete eight-edit preview, applied outcome, rollback, and console-error absence. |
