...
| 8 | Agent activity is visible in the interface. | Verified | Shared handlers feed the activity list and graph highlighting; the browser journey checks causal failure, visible sacred pin, patch preview, applied outcome, rollback, and console-error absence. |
...
| 12 | Unit, integration, production-build, and browser tests pass through `prepare.py`. | Verified locally and in CI | `python3 prepare.py --quick` passes locally; GitHub Actions performs a clean install, zero-vulnerability audit, quick acceptance, and production Chrome journey on Node 22.13. Full local acceptance reaches only the separate live discovery gate below. |
