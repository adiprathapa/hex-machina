...
The headless family benchmark, JSONL exporter, streaming rollout service, and single/vector Python adapters invoke these handlers directly across 72 deterministic variants spanning two causal rules. They never use a parallel mock tool surface; the same validation, solver, simulator, reward recorder, and stale-write protection remain in force. A shared public-observation projection prevents `inspect_spell`, reset/step responses, replay transitions, JSONL, and Python adapters from exposing private simulator role assignments or answer-key edges. Vector slots own isolated handler instances and processes. The environment catches handler failures only at its rollout boundary, converting them into replayable negative-reward transitions while preserving normal UI/WebMCP error behavior.
