...
A reproducible benchmark command runs the transparent reference policy across all 72 tasks and emits per-episode JSON receipts plus split means. A contrast suite proves reward separation on held-out tasks: grounded completion scores 23, unsafe mutation-first completion 18, safe but incomplete diagnosis 6, and canonical-ID memorization −8. A separate JSONL exporter produces replay-complete transitions with scalar rewards, terminal flags, before/after graphs, and stable state keys for offline analysis. For online evaluation, a strict stdin/stdout JSONL service exposes correlated reset, step, describe, and snapshot operations; dependency-free Python adapters provide both single-environment and parallel vectorized Gymnasium-shaped signatures while every slot delegates to isolated production handlers. This makes environment drift visible and external policy integration practical without presenting scripted controls as learned results.
