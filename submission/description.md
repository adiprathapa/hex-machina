...
The research layer contains 72 deterministic variants in two causal families: Moonflower's 48 carrier-path tasks use 32/8/8 train/validation/test splits, while Resonant Aviary's 24 feedback-cycle tasks use 16/4/4. Every variant remaps opaque IDs, order, layout, effect ID, and prompt wording. Agent-visible observations omit private simulator roles, causal rule IDs, and answer-key edges. A transparent policy grounds protected intent from natural-language constraints plus inspected rune text and still earns 23/23 on held-out variants from both rules, while a memorized training ID fails safely. This demonstrates cross-rule grounding, not generalized RL training.

A reproducible benchmark command runs the transparent reference policy across all 72 tasks and emits per-episode JSON receipts plus split means. A contrast suite proves reward separation on held-out tasks: grounded completion scores 23, unsafe mutation-first completion 18, safe but incomplete diagnosis 6, and canonical-ID memorization −8. A JSONL exporter produces replay-complete transitions with scalar rewards, terminal flags, before/after graphs, stable state keys, and explicit variant metadata for offline analysis. An independent verifier reconstructs every task and replays every action through production handlers, rejecting altered metadata, actions, rewards, observations, results, terminal scores, or duplicate scenarios. For online evaluation, a strict stdin/stdout JSONL service exposes correlated reset, step, describe, and snapshot operations; dependency-free Python adapters provide both single-environment and parallel vectorized Gymnasium-shaped signatures while every slot delegates to isolated production handlers. This makes environment and dataset drift visible and external policy integration practical without presenting scripted controls as learned results.
