...
- Agent Gym currently evaluates two semantic task families with 72 deterministic opaque-ID and prompt variants. Held-out success crosses an unshielded carrier rule and a reachable feedback-cycle rule, but does not establish broad transfer to unrelated domains. The environment does not itself train an LLM.
- JSONL trajectories are generated by a transparent scripted reference policy. It grounds protected intent with lexical evidence from the public task and inspected runes; private simulator semantics are excluded from its observations. The trajectories are useful for replay, schema integration, and pipeline prototyping, but are not expert demonstrations from a learned policy and should not be treated as evidence of model improvement. The live subprocess/Python adapters enable single and process-vectorized online rollouts but do not provide multi-host orchestration or a learning algorithm.
