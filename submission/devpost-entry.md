...
The reproducible benchmark command runs the transparent reference policy over all 48 variants and emits JSON episode receipts and split means. It currently reports 48/48 completions at 23/23. A held-out contrast suite proves that the reward separates grounded completion (23), unsafe mutation-first completion (18), safe but incomplete diagnosis (6), and canonical-ID memorization (−8); its aggregate results are visible in the product. A JSONL exporter emits replay-complete transitions with observations, scalar rewards, termination/truncation flags, state keys, results, and errors. A strict streaming JSONL service and dependency-free Python adapter also let an external trainer drive those same production transitions online with Gymnasium-shaped reset/step results. Invalid calls are scored without crashing the rollout, and unfinished episodes truncate deterministically at 32 actions. These validate the environment and scripted controls, not a learned-policy result.
