...
The reproducible benchmark command runs the transparent reference policy over all 48 variants and emits JSON episode receipts and split means. It currently reports 48/48 completions at 23/23. A JSONL exporter emits replay-complete transitions with observations, scalar rewards, termination/truncation flags, state keys, results, and errors. A strict streaming JSONL service and dependency-free Python adapter also let an external trainer drive those same production transitions online with Gymnasium-shaped reset/step results. Invalid calls are scored without crashing the rollout, and unfinished episodes truncate deterministically at 32 actions. These validate the environment and baseline, not a learned-policy result.
