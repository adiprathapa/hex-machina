...
| Callback results may be any JSON-serializable value and are serialized by the browser for the caller. | Handler results contain plain structured data only: graph versions, IDs, coherent internal and boundary edge sets, filter counts, scenario status, ordered events and causal paths, typed minimal side-effect subgraphs, positive and negative rule premises, counterfactual edge-necessity checks, cycles, type violations, assertions, explicit and validated patch preconditions, diffs, verification casts, and revert tokens. |
