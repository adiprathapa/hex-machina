...
Hexmend does not ask an agent to own or infer application state. The client application owns a versioned `SpellGraph`; tools receive bounded inputs and call the same pure domain operations as the fallback interface.
...
- `src/scenarios/agent-gym-family.ts`: 96 seeded variants across three causal families, with opaque role/edge/effect IDs, benign topology distractors, stable shuffling, layout jitter, prompt paraphrases, and disjoint train/validation/test splits.
...
- `src/tools/definitions.ts`: versioned serializable action definitions that generate both the generic Agent Gym manifest and identifier-narrowed canonical WebMCP registrations.
- `src/tools/handlers.ts`: runtime-validated semantic operations independent of the browser adapter, including a typed result-presentation channel and stale-safe one-use patch rollback; TypeScript types and JSON Schemas are never treated as an agent-input security boundary.
...
The family generators prevent a policy from succeeding by memorizing canonical IDs or assuming one exact family topology. Unit tests solve held-out validation and test variants only after grounding protected intent from natural-language constraints and inspected rune text; neither the policy nor `inspect_spell` receives the private semantic role map. Reusing a training variant's protected-node ID on a test graph fails safely and records a negative reward. Each generated graph also activates a nonempty seeded subset of three valid decoy edges among otherwise dormant runes. These one-to-three-edge subgraphs are disjoint from the private causal route, producing at least three observed topologies in every family while leaving simulator truth unchanged. Moonflower's 48 variants use an unshielded carrier failure, Resonant Aviary's 24 variants use a reachable feedback-cycle failure, and Clockwork Orchard's 24 variants use an absent temporal guard. All 96 variants validate and reproduce byte-identically from family, split, and index.
...
`npm run --silent gym:dataset -- --split=test` emits newline-delimited `hexmend-agent-gym-episode/v2` records. Each line is a standalone training episode: environment and observation protocols, identifier-neutral action manifest, human-visible objective and constraint, initial public observation and FNV-1a state commitment, explicit variant metadata, transitions, and terminal receipt. Omitting the split exports all 96 baseline rollouts without writing files or contacting an external service. Piping that output to `npm run --silent gym:verify` reconstructs every selected variant, authenticates the complete reset context and manifest, and then replays every action through production handlers. The verifier rejects changed prompts, initial graphs/keys, tool definitions, metadata, transitions, terminal receipts, or duplicate scenarios, exits nonzero on alteration, and caps untrusted input at 20 MiB and 1,000 episodes.

`npm run --silent gym:serve` exposes the same environment as a stateful newline-delimited subprocess protocol. Correlated `describe`, `reset`, `step`, and `snapshot` operations keep transport failures distinct from scored agent mistakes and never write logs to stdout. `describe` and reset receipts include `hexmend-tool-manifest/v1`: the exact action envelope plus seven descriptions, JSON Schemas, and safety annotations. WebMCP registrations are derived from those same definitions with canonical identifier enums added; the generic training form omits task-specific identifiers to avoid leaking a held-out graph's roles or side effect before interaction. Contract tests compare the registered definitions with the shared manifest directly. Reset accepts either an explicit split/family/index or a bounded 32-bit sample seed. The versioned xorshift32 sampler deterministically selects across every task, or inside a requested family for curriculum control; seeded resets default to `train`, preserve split isolation, and return an auditable sampler receipt. The dependency-free Python adapter exposes Gymnasium-shaped seeded reset and step signatures while delegating every transition to the TypeScript process, avoiding a second simulator or reward implementation. Cross-process tests prove identical seed/options reproduce the same held-out observation and task.

`HexmendVectorEnv` composes isolated adapters into a process-level vector environment. Each slot owns its own TypeScript bridge, graph, issued patch capabilities, reward recorder, and trajectory; a bounded thread pool executes reset, step, describe, snapshot, and close calls concurrently while preserving input slot order. Strict batch lengths and explicit split/index pairing prevent silent task misalignment. A scalar vector seed expands deterministically across slots with 32-bit wraparound; an explicit seed sequence permits exact task assignment. Cross-process tests prove seeded vector resets repeat byte-identically, then run three different opaque scenarios together, inject an invalid action into only one slot, and prove rewards, acceptance flags, state keys, and trajectory lengths remain isolated.

`npm run --silent gym:preferences` converts the five executable behavioral controls into 64 train-split groups. Every v2 record shares one authenticated task/reset/tool context and contains grounded, mutation-first, diagnosis-only, constraint-violating, and memorized-ID trajectories ranked by their deterministic 23/18/6/4/−8 rewards. Centering around the 8.6 group mean yields 14.4/9.4/−2.6/−4.6/−16.6 advantages, and the strict ordering yields all ten positive-margin preference pairs. Every candidate exposes both `constraintViolation` and `constraintPreserved`, so a completed human-overruling trajectory cannot masquerade as ordinary success. `gym:preferences:verify` regenerates every labeled policy through fresh production handlers and compares the complete group, so altered trajectories, safety/invalid/constraint evidence, rankings, advantages, or margins fail closed.

`HexmendPreferenceDataset` is the dependency-free Python consumption boundary for that JSONL. Its `verify()` method invokes the canonical TypeScript regeneration verifier before use; `groups()` validates and streams one bounded record at a time; and `pairs()` projects all 640 train comparisons without materializing a second corpus. Both iterators accept exact split and family filters, retain canonical file order, and fail on zero matches so curriculum or evaluation typos cannot silently produce an empty epoch. Every projected `hexmend-agent-gym-preference-pair/v2` record retains the shared task, reset observation and state key, identifier-neutral action manifest, complete chosen/rejected trajectories, explicit constraint labels, and exact reward margin. Cross-language tests cover all 160 validation pairs, a family-restricted 80-pair stream, zero-match behavior, and tamper rejection.

`npm run --silent gym:policies` runs five deterministic controls over all held-out test variants. Grounded reference behavior scores 23 with 100% completion and no unsafe episodes; mutation-before-explanation also completes but scores 18 with a 100% unsafe-episode rate; diagnosis-only stops safely at 6; constraint-violating completion scores 4 with a 100% violation rate; canonical-ID memorization records four invalid actions and scores −8. The product renders these checked aggregate rewards as a compact separation table. This is evidence that the rubric responds to grounding, safety, completeness, and human intent. It is not a claim about learned models.

This makes Hexmend useful as a cross-rule online evaluation harness and trajectory generator today. It is not yet a general RL training service: broader semantic diversity, distributed rollout orchestration, and learned-policy experiments remain subsequent research work.
