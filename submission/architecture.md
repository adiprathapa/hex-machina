...
`npm run --silent gym:preferences` converts the five executable behavioral controls into 64 train-split groups. Every record shares one authenticated task/reset/tool context and contains grounded, mutation-first, diagnosis-only, constraint-violating, and memorized-ID trajectories ranked by their deterministic 23/18/6/4/−8 rewards. Centering around the 8.6 group mean yields 14.4/9.4/−2.6/−4.6/−16.6 advantages, and the strict ordering yields all ten positive-margin preference pairs. `gym:preferences:verify` regenerates every labeled policy through fresh production handlers and compares the complete group, so altered trajectories, safety/invalid/constraint evidence, rankings, advantages, or margins fail closed.

`HexMachinaPreferenceDataset` is the dependency-free Python consumption boundary for that JSONL. Its `verify()` method invokes the canonical TypeScript regeneration verifier before use; `groups()` validates and streams one bounded record at a time; and `pairs()` projects all 640 train comparisons without materializing a second corpus. Every projected `hex-machina-agent-gym-preference-pair/v1` record retains the shared task, reset observation and state key, identifier-neutral action manifest, complete chosen/rejected trajectories, and exact reward margin. Cross-language tests cover all 160 validation pairs and tamper rejection.
