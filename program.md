...
The game is also a legible agent-evaluation environment. Its typed graph is the observation, the seven semantic tools are the action space, application-owned handlers define deterministic transitions, and the human-constraint-preserving journey supplies a reproducible reward signal. The canonical lesson remains the judge-facing demonstration; an Agent Gym protocol exposes reset, step, score, and an exportable trajectory for evaluating tool-using policies. Three deterministic causal families now provide 96 opaque-ID, topology, layout, and prompt variants across disjoint train, validation, and test splits: Moonflower tests an unshielded amplified carrier path, Resonant Aviary tests a reachable directed feedback cycle, and Clockwork Orchard tests an action fired without its required temporal guard. Seeded benign subgraphs vary each family's visible topology without entering its private answer-key route. This is evidence that a grounded policy transfers across three simulator rules and ignores causal distractors, not a claim that the application already trains models or generalizes broadly.
...
15. The same production handlers power a deterministic Agent Gym episode with reset/step semantics, explicit reward and termination flags, bounded rollouts, replayable public before/after observations, visible scoring, negative rewards for invalid or premature actions, standalone JSONL episodes containing the task prompt, initial public graph/state commitment, and action manifest, an independent deterministic verifier that authenticates reset context before replay, seeded task sampling with family-restricted curricula, streaming JSONL/Python adapters for online and vectorized training loops, a dependency-free bounded Python reader for verified group and chosen/rejected preference iteration with exact split/family filtering, a versioned self-describing action manifest generated from the same definitions as WebMCP registration, executable contrast policies proving reward separation, and verifier-backed group-relative/preference records with centered advantages and positive chosen/rejected margins. Private simulator roles, answer-key edges, task-specific identifiers, rule-revealing metadata, and pre-cast diagnostic assertions must never appear in the generic training manifest or agent observations. Patch IDs must be proposal-issued capabilities, never guessable write authorization.
...
- Additional task families beyond the three implemented causal rules.
