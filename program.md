...
15. The same production handlers power a deterministic Agent Gym episode with reset/step semantics, explicit reward and termination flags, bounded rollouts, replayable before/after observations, visible scoring, negative rewards for invalid or premature actions, exportable JSON/JSONL trajectories, a streaming JSONL/Python adapter for online training loops, and executable contrast policies proving reward separation. Patch IDs must be proposal-issued capabilities, never guessable write authorization.
